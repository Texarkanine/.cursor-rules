...
Determine what to classify. If `memory-bank/active/milestones.md` exists, read it — the classification target is the description of the first unchecked (`- [ ]`) milestone plus that milestone's Per-milestone done and risks row or heading block if present. Otherwise, the classification target is the user's task input.
