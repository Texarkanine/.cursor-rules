...
- Carve-out definition: `.cursor/rules/shared/always-tdd.mdc` (Niko ships it always-on). Prose/policy units use the `N/A` form under Tests first.
...
1. [Step 1 - component, TDD cycle]
    - Files: [file paths]
    - Tests first: [test file + case(s) to write and watch fail; or `N/A — prose/policy artifact (TDD carve-out)`]
