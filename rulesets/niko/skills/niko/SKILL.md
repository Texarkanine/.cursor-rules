...
2. Delete sub-run ephemeral files from `memory-bank/active/`:
    - **Delete:** `tasks.md`, `activeContext.md`, `progress.md`, `creative/` (if present), `troubleshooting/` (if present), `.qa-validation-status`, `.preflight-status` (if present)
...
3. Delete from `memory-bank/active/`: `tasks.md`, `activeContext.md`, `troubleshooting/` (if present), `.qa-validation-status` (if present), `.preflight-status` (if present).
