...
Use when:
- Switching AI harnesses mid-phase (e.g., Cursor → Claude Code)
- The context window is long and you want to checkpoint before starting a fresh session
- Gracefully exiting a long-running phase
...
Read every ephemeral file that exists:
...
- `memory-bank/active/creative/*.md` (if any)
- `memory-bank/active/reflection/*.md` (if any)
...
**Do not fabricate state.** Only write what you actually know from the current conversation. If you are uncertain whether something was already written, err on the side of writing it — redundant truth is harmless; missing state is not.
