...
8. Cost is the output price in USD per million tokens, from https://cursor.com/docs/models-and-pricing. The mapping points each slug at a pricing row and at a BenchLM model slug.
...
3. The nine slugs this session can spawn each get a BenchLM category mean and share one initial tier, so they are in the pool. The operator can split that tier later by editing the catalog.
