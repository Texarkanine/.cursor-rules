...
1. **Create intent-clarification skill file (Claude Code)**
...
2. **Create intent-clarification rule file (Cursor)**
    - Files: `rulesets/niko/niko/core/intent-clarification.mdc`
    - Changes: Same content as step 1 but with `.cursor/rules/shared/niko/...` path references instead of `.claude/skills/...` paths. Frontmatter uses `alwaysApply: false` (Cursor rule format).
    - Note: Follows the dual-format pattern established by `complexity-analysis` (`.claude/skills/complexity-analysis/SKILL.md` + `rulesets/niko/niko/core/complexity-analysis.mdc`)

3. **Update `/niko` state machine (both copies)**
    - Files:
      - `.claude/skills/niko/SKILL.md` (Claude Code version)
      - `rulesets/niko/skills/niko/SKILL.md` (Cursor source version)
    - Changes (applied to both, with appropriate path references):
      - Renumber Step 5 (Resume) → Step 6
      - Renumber Step 6 (Classify) → Step 7
      - Insert new Step 5 (Clarify Intent) with `Load:` directive pointing to intent-clarification skill/rule
...
4. **Update complexity analysis (both copies)**
    - Files:
      - `.claude/skills/complexity-analysis/SKILL.md` (Claude Code version)
      - `rulesets/niko/niko/core/complexity-analysis.mdc` (Cursor source version)
...
5. **Update milestones reference doc**
    - Files: `rulesets/niko/niko/memory-bank/active/milestones.mdc`
    - Changes: Update "Step 2a → Step 6 (Classify)" reference to "Step 2a → Step 7 (Classify)"
    - Note: The synced copy at `.claude/rules/shared/niko/memory-bank/active/milestones.md` will be updated by ai-rizz sync
...
7. **Review and validate**
    ...
    - Verify rulesets and .claude copies are consistent (same content, different path references)
...
- **Step renumbering ripple effects**: Renumbering Steps 5→6 and 6→7 affects references in `/niko` SKILL.md (both copies), milestones.mdc, and potentially other files. Mitigation: systematic search-and-replace with manual verification of each reference.
- **Dual-format maintenance**: Every content change must be applied to both `.claude/skills/` (Claude Code) and `rulesets/` (Cursor) copies, with appropriate path reference translation. Mitigation: edit one copy first, then translate paths for the other.
