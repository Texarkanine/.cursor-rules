...
3. **Tighten Plan phase TDD authorship — put the requirement in the emitted artifact, not another instruction**
   ...
   - The `- Tests first:` substep must carry an explicit carve-out escape hatch (e.g. `N/A for prose & policy artifacts`) so the template cannot push change-detector tests onto documentation units. Without it this fix creates the exact failure mode this task's own plan avoids.
