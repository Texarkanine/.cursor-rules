...
- [ ] **OQ-1 (primary, phrasing & placement): What exact text goes in the rule, and where does it live?** → Explored in `memory-bank/active/creative/creative-commit-autonomy.md` (iteration 2). **Unresolved** — operator is selecting phrasing collaboratively. Iteration 2 adds C6 (per-workflow/per-skill headers, with variants a/b/c/d), reframes the rule as a **general Niko principle** ("Workflow Invocation is Explicit Consent") rather than a Cursor-specific patch, and explicitly addresses the empirical sub-command failure mode (`/niko-X` not recognized as inheriting consent from `/niko`). New recommendation: C6d hybrid (core bullet + canonical reference + per-file headers). Awaiting operator answers to seven numbered questions. Sub-questions bundled:
