...
- **Tier B — in the repository, different commit.** `git fetch origin pull/{N}/head`, then `git worktree add --detach` into a temporary path. Additive, shares the object store, downloads nothing already present, and leaves the operator's checkout untouched. Remove the worktree when done OR consider just putting the worktree in a tempdir (mktemp) and letting it clean up naturally - maybe safer than trying to orchestrate descructive git operations.
...
- **Tier D — clone, last resort.** Only for work no single-file fetch can do: searching the whole tree for callers, or running tests. Check `gh api repos/{o}/{r} --jq .size` first — it answers in KB, and it separates a 769 KB repository from a 62 GB one before any transfer begins. Above the operator's tolerance, stay on Tier C and say why. When cloning is genuinely warranted, use `--filter=blob:none --depth 1 --single-branch` into a `mktemp -d`.
